#!/usr/bin/env bash
# pm46 v2 Phase 3 — CAPX cap-eat-alive matrix
# 17 defenders × 30 RANDOM seeds × 1 game each = 510 games (P6 fork)
#
# Per plan §6 Phase 3, P6 wrapper fork:
#   - Separate from pm46_v2_a_solo_matrix_corrected.sh (ABS-only).
#   - Uses [CAPX_CAP_EATEN] / [CAPX_A_DIED] events.
#   - CAPX_GATE_HORIZON=8 patch enabled by default in agent code.
#
# Output:
#   experiments/results/pm46_v2/capx_matrix.csv
#   experiments/results/pm46_v2/logs_capx_matrix/
#
# Run from repo root:
#   bash experiments/rc_tempo/pm46_v2_capx_matrix.sh [CAPX_MIN_MARGIN]
...
MARGIN="${1:-0}"
CSV="$OUT_DIR/capx_matrix_m${MARGIN}.csv"
...
echo "=== pm46 v2 CAPX matrix (CAPX_MIN_MARGIN=$MARGIN) ==="
