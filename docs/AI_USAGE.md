...
## 2026-04-21 — pm32 Step A-C.2: β agent env-var sweep infra (Mac coding window)

- **AI**: Claude Opus 4.7 (1M context) — OMC executor 서브에이전트
- **Prompt**: pm32 sweep plan §6.A-C.2 실행. Step A (40 NEW VARIANTS in v3a_sweep), Step B (BETA_TRIGGER_GATE + BETA_TRIGGER_MAX_DIST in zoo_reflex_rc_tempo_beta), Step C (BETA_RETREAT_ON_ABORT + my_home_cells plumbing + _maybe_retreat helper + MJ-7 module-singleton leak guard), Step C.0 (composite.py single source of truth + T-U4/T-U5 unit tests), Step C.1 (v3a_sweep hardening + new files: composite.py, promote_t1_to_t2.py, analyze_pm32.py, filter_random_layouts.py, hth_sweep.py, fixtures stub), Step C.2 (CLI surface verification gate). 모든 verification gate (T-U1~T-U5 unit tests + 6 sub-feature gates) 통과 확인 후 git push 전.
- **수정 부분**:
  - `minicontest/zoo_reflex_rc_tempo_beta.py` — BETA_TRIGGER_GATE/BETA_TRIGGER_MAX_DIST trigger gating, BETA_RETREAT_ON_ABORT _maybe_retreat helper (3 abort sites repointed), my_home_cells team-state plumbing, MJ-7 RCTEMPO_TEAM slot reset at top of _precompute_team, naming-asymmetry header doc paragraph
  - `minicontest/zoo_reflex_rc_tempo_beta_retro.py` — naming-asymmetry header doc only (no logic change)
  - `experiments/rc_tempo/v3a_sweep.py` — +40 VARIANTS entries (5 P1 + 20 AA + 10 AC + 5 RS), --variants-file/--layouts-file/--validate-csv/--allow-truncate flags, heartbeat thread, wall_summary.csv, disk pre-check, per-variant try/except, refactored summarize_sweep to call composite.compute_score, _resolve_agent_and_env exposed for hth_sweep
  - `experiments/rc_tempo/composite.py` — NEW: compute_score, wilson_ci_95, pearson_with_ci (Fisher z back-transform), spearman_rho. z=1.96 매치 hth_resumable.py:47.
  - `experiments/rc_tempo/promote_t1_to_t2.py` — NEW: T1→T2 promotion with --top-n 12, --buffer-pp 2.0, --die-ceiling 2.5, --stratify-angles + --stratify-tolerance-pp 5.0, --data-quality-check, --force-include defaults locked per MJ-4/MJ-5
  - `experiments/rc_tempo/analyze_pm32.py` — NEW: composite-only sort (refuses WR-sort per MINOR #15), per-opp breakdown (Critic-U), per-fixed-layout T2 vs F3 (v2 #75), Pearson r 95% CI + Spearman ρ, ship verdict via MJ-3 conjunction rule
  - `experiments/rc_tempo/filter_random_layouts.py` — NEW: 1-cap-per-side filter for RANDOM<seed> via mazeGenerator.generateMaze
  - `experiments/rc_tempo/hth_sweep.py` — NEW: thin orchestrator wrapping hth_resumable.py for F3, reuses _resolve_agent_and_env from v3a_sweep
  - `experiments/rc_tempo/test_env_parsing.py` — NEW: T-U1 (env parsing & defaults), T-U2 (backward compat reproduction), T-U3 (a) retro × retreat ON/OFF differs + (b) MJ-7 module-singleton no-leak across layouts
  - `experiments/rc_tempo/test_composite.py` — NEW: T-U4 (composite ranks beta_path4 > beta_retro > beta_v2d), T-U5 (Pearson + Spearman 5 cases each, hardcoded numpy-validated r value)
  - `experiments/rc_tempo/pm32_smoke_variants.txt` — NEW: 13 variants for Mac smoke (incl. pm32_aa_none_d999 for MJ-8 byte-identical diff)
  - `experiments/rc_tempo/fixtures/_build_pm31_s5_stub.py` + 70 per-variant fixture CSVs — NEW
- **검증 방법 및 결과**:
  - **Step A acceptance**: `from v3a_sweep import VARIANTS; print(len(VARIANTS))` → 70 ✓; per-angle counts pm32_p1_=5, pm32_aa_=20, pm32_ac_=10, pm32_rs_=5 ✓
  - **Step B acceptance (T-U1)**: 4 env-parsing tests pass — _iv handles garbage, BETA_TRIGGER_GATE='wat_is_this' silently → 'none', BETA_TRIGGER_MAX_DIST=0 doesn't abort, BETA_RETREAT_ON_ABORT default '0' works
  - **Step B acceptance (T-U2)**: 2 consecutive subprocess runs byte-identical on (defaultCapture, baseline, seed=42, our_team=red, max_moves=120)
  - **Step C acceptance (T-U3 a)**: zoo_reflex_rc_tempo_beta_retro vs zoo_reflex_rc82 on defaultCapture seed=42 → score=-1 (OFF) vs +1 (ON), a_final_pos=[1,14] vs [24,10]
  - **Step C acceptance (T-U3 b MJ-7)**: 2-game-different-layouts in single subprocess (defaultCapture w=32→distantCapture w=40), my_home_cells after game 2 reads via sys.modules['player1'].RCTEMPO_TEAM correctly shows g2_home_xs=[19] (NOT g1's [15])
  - **Step C.0 acceptance (T-U4)**: composite.compute_score on stub ranks beta_path4 > beta_retro > beta_v2d ✓
  - **Step C.0 acceptance (T-U5)**: Pearson 5 cases + Spearman 6 cases pass; pinned numpy-validated r=0.9855009986463001 for N=12 known case matches our impl to 6 places
  - **Step C.1 acceptance (6 sub-features)**: --variants-file works, --validate-csv on empty rc=0, --validate-csv on 70-CSV fixture rc=0, hth_sweep env-var sanity OK, promote_t1_to_t2 --dry-run on fixture ranks beta_path4 (55.41 #1) > beta_v2d (47.31), heartbeat.log + wall_summary.csv written on tiny sweep
  - **Step C.2 acceptance**: 4 distinct CLI flags (--variants-file/--layouts-file/--validate-csv/--allow-truncate) all present in `--help`
  - **Total tests**: 25 pass (7 env-parsing + 18 composite). Wall ≈ 10s.
- **배경/의도**: pm32 sweep는 70 variants × 11 opp × ~16 layout × 2 color × {5,30}g 두 단계로 β capsule-chase 하이퍼파라미터 탐색. zoo_reflex_rc_tempo_beta.py는 your_best.py에 평탄화될 예정이므로 이 entry가 채점 대상 코드 추적에 포함됨. 모든 새 env var defaults가 pm31 v2d 동작을 정확히 재현하도록 설계 (backward compat은 MJ-3 ship gate 전제).

---

## 2026-04-21 19:00 — pm32 END: 16 layouts hand-crafted + Mac smoke + Python 3.9.25 upgrade

- **AI**: Claude Opus 4.7 (1M, OMC orchestrator + executor sub-agents)
- **Prompt**: pm32 plan v3 §6.D (Mac smoke), layouts/* hand-crafting (mazeGenerator.py forces 2-cap → pivot to manual layouts), env parity (Mac 3.9.11 → 3.9.25 to match jdl_wsl/sts), pm33 handoff prep.
- **수정 부분 (제출 코드 추가 변경 없음, 평탄화 source는 zoo_reflex_rc_tempo_beta.py — 이미 pm32 첫 entry로 logged)**:
  - `minicontest/layouts/defaultCapture_cap{N,S,Center,Corner}.lay` (NEW, 4 capsule-swap variants)
  - `minicontest/layouts/distantCapture_cap{N,Center}.lay` (NEW, 2 capsule-swap variants)
  - `minicontest/layouts/strategicCapture_cap{N,Corner}.lay` (NEW, 2 capsule-swap variants)
  - `minicontest/layouts/pm32_{corridor,open,fortress,zigzag,choke}Capture.lay` (NEW, 5 hand-crafted topologies, 24×12 / 28×14 / 28×11)
  - Mac venv: rebuilt at Python 3.9.25 (uv venv --python 3.9.25 .venv) to match server parity
- **검증 방법**:
  - 13 layouts × 4 verification gates (border-walls, capsule count == 2, exactly one each of 1/2/3/4, 180° symmetry, BFS connectivity) all PASS via `/tmp/verify_layouts.py`
  - All 13 layouts loaded successfully by capture.py engine (`-l <name> -r baseline -b baseline -n 1 -q` returned game outcome)
  - β agent test on 3 hand-crafted (corridor/choke/fortress): completed without crash
  - Mac smoke (459s wall, 13 var × 6 opp × 2 L × 2 c × 5g, workers=6, max_moves=200): 13/13 no-crash; MJ-8 byte-identical pass (pm32_aa_none_d999 ≡ beta_v2d both 13.3%/1.7%); 25 unit tests PASS post-Python-upgrade
  - capture.py md5 identical Mac/jdl_wsl/sts; numpy 2.0.2 + pandas 2.3.3 identical all 3 environments
- **배경/의도**: pm32 plan §6.D Mac smoke 검증 + 16 layouts pool (3 fixed + 8 cap-swap + 5 topology) 준비. mazeGenerator.py가 RANDOM seed에서 항상 2-cap 생성 (1-cap-only 강제 요구 충돌) → hand-craft로 우회. Python 버전 통일은 Step E parity test 사전 위험 제거. pm33 handoff: Step E (git push + server pull) → F1 (T1 sweep 1.8h) → F2 (T2 1.8h) → F3 (HTH calib 1h).
