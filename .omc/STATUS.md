...
🔴 **M6 budget overshoot — pm13 M4b-4 measured 10.5s/match (8-worker contention)** — Stage 1 Pre-α baseline was 7.74s/match (1-worker). Under 8-worker parallel load the per-match effective time rose 36% due to subprocess/OS contention. Re-extrapolating to STRATEGY §6.6 full spec: Phase 2a 10 gens × 40 pop × 264 games / 8 workers = **~38.5h**, Phase 2b 20 gens × 40 pop × 224 games / 8 workers = **~65h**, **M6 total ≈ 103h ≈ 4.3 days** (vs STRATEGY's "~20h, 1 day" estimate — 5× overrun). Root cause probably the immutable capture.py 4-loop (every `-n 1` run_match call actually plays 4 capture.py games because of `main()`'s `for i in lst` harness) which STRATEGY's compute budget did NOT account for. **Options under consideration**: (A) bypass 4-loop by importing capture.py's `runGames` directly in run_match.py (~4× speedup potential, ~30min-1h work, correctness risk); (B) scale M6 down (pop 40→20, games 264→132, gens 10+20→5+10) to ~13h at cost of statistical power; (C) accept 4-day wall and plan overnight+weekend schedule; (D) hybrid (A) + mild scale-down to safely hit ~20h. Decision pending user input — tier policy means we split M6 into M6-a/b/c/d anyway, so worst case we stop at M6-a (Phase 2a smoke) and reassess.
